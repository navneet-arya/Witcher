...
def print_help():
	...
	print("{0}Boomerang, v0.0.1 - made by @Navneet arya{1}\n".format(BOLD, END))
	...
	print("\n\n{0}Usage:{1} $boomerang {2}[file_name]{1}".format(UNDERLINE, END, YELLOW))
	print("\n$python3 {0}test.py{1} => $boomerang {0}test.py{1}".format(YELLOW, END))
	print("\nIf you just want to query Stack Overflow, use the -q parameter: $boomerang -q {0}What is an array comprehension?{1}\n\n".format(YELLOW, END))
...
def confirm(question):
	...
	while True:
		print("{0}{1}{2}{3}{4}".format(BOLD, CYAN, question, prompt, END))
		choice = input().lower()
		if choice in valid:
			return valid[choice]
...
def get_bs4_object(url):
	...
	try:
		...
		sys.stdout.write("\n%s%s%s" % (RED, "Boomerang was unable to fetch Stack Overflow results. "
	...
	def _handle_input(self, input):
		if input == "enter": # View Answers
			...
			if url != None:
				...
				# pile = urwid.Filler(urwid.Pile([urwid.Text(u"Hello"), urwid.Text(u"World")]))
				...
				# linebox.draw_screen()
				self.main_loop.widget = urwid.Frame(body=urwid.Overlay(linebox, self.content_container, "center", ("relative", 90), "middle", 25), footer=menu)
